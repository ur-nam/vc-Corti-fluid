function  El = lumped_mass(Nd, El,  MP)
% The masses of BM and OC are lumped to the elements along the z-axis, BMz and DCz

    
% %     mthick_BMz = thickness_BMz(Nd, El, eBMz, MP);
    
    eBMza = strcmp('BMza',El.name);
    eBMzp = strcmp('BMzp',El.name);
    eBMz = eBMza | eBMzp;
    mthick_BMza = thickness_BMz(Nd, El, eBMza, MP);
    mthick_BMzp = thickness_BMz(Nd, El, eBMzp, MP);
    
    eRLz = strcmp('RLz',El.name);
    eX = eRLz;
    mthick_RLz = thickness_Z(Nd, El, eX, 'thick_BMz', MP);
    mfactor = 0.2*mthick_RLz./El.dim(eX,2); % This is not good. This must be defined in model_geometry and in MP.    
       
    
    El.rho(eRLz) = El.rho(eRLz) + 0*mfactor*1e-3;
    
    eTMz0 = strcmpi(El.name,'TMz0');
    mthick_TMz0 = thickness_Z(Nd, El, eTMz0, 'thick_TMza', MP);
    
    eTMz1 = strcmpi(El.name,'TMz1');
    mthick_TMz1 = thickness_Z(Nd, El, eTMz1, 'thick_TMzb', MP);    
    ...
    if mean(El.type(eBMz))>2.5,
        ...
        mfactor = (1+MP.rOCmass)*mthick./El.dim(eX,2); % factor 1.5 in order to include supporting cells in OC               
              
        
        
        El.rho(eX) = El.rho(eX) + mfactor*1e-3;
...
function thick_BMz = thickness_BMz(Nd, El, eX, MP)
    
    qJH = consider_geometrical_lgradient_og(MP);
...
function thick_TMz = thickness_Z(Nd, El, eX, prop, MP)
    ...
    zi = mean(Nd.Z(nds),2);
    zz = (-0.5*MP.length_BM:MP.dZ:0.5*MP.length_BM)' + mean([Nd.Z(El.Nd1(eX)); Nd.Z(El.Nd2(eX))]);
