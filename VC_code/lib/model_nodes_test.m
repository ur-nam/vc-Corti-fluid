function [Nd, MP] = model_nodes_test(MP)
...
function     Nd = assign_nodal_coordinates(p, MP)
    ...
    Nd.X = zeros(nz,nBM+18);
    Nd.Y = zeros(nz,nBM+18);
    Nd.Z = zeros(nz,nBM+18);
    
%     nBMp = floor((1-MP.DC_root_location)*MP.NX)+1;
%     nBMa = floor(MP.OPC_root_location*MP.NX)+1;
    %hardcoding discretization
    nBMp = 7;
    nBMa = 5;    
    ...
    for iz=1:nz    
        ...
        nodes_IPC_internal = linspace(p.A0(iz,1),p.CC(iz,1),5); nodes_IPC_internal([1,end]) = [];
        nodes_OPC_internal = linspace(p.A1(iz,1),p.CC(iz,1),5); nodes_OPC_internal([1,end]) = [];

        Nd.X(iz,:) = [nodes_arcuate, nodes_IPC_DC_roots, nodes_pectinate,...
                    nodes_IPC_internal,nodes_OPC_internal, p.DD(iz,1), ...
                    p.CC(iz,1), p.BB(iz,1), ...
                    0.5*(p.CC(iz,1) + p.BB(iz,1)), ...
                    p.FF(iz,1), p.E1(iz,1), p.E2(iz,1), p.E3(iz,1), ...
                    p.E4(iz,1), p.E5(iz,1), p.E6(iz,1),p.PP(iz,1)];

        nodes_IPC_internal = linspace(p.A0(iz,2),p.CC(iz,2),5); nodes_IPC_internal([1,end]) = [];
        nodes_OPC_internal = linspace(p.A1(iz,2),p.CC(iz,2),5); nodes_OPC_internal([1,end]) = [];

        Nd.Y(iz,:) = [zeros(1,nBM),...
                    nodes_IPC_internal,nodes_OPC_internal, p.DD(iz,2), ...
                    p.CC(iz,2), p.BB(iz,2), ...
                    0.5*(p.CC(iz,2) + p.BB(iz,2)), ...
                    p.FF(iz,2), p.E1(iz,2), p.E2(iz,2), p.E3(iz,2), ...
                    p.E4(iz,2), p.E5(iz,2), p.E6(iz,2),p.PP(iz,2)];
    end
   
%     z_tilt = -0.25*MP.tilt_OHC*10;      % O--+------X
    z_tilt = -0.125*MP.tilt_OHC*10;      % O--+------X
...
% %     %.oOo.oOo.oOo.oOo.oOo.oOo.oOo.oOo.oOo.oOo.oOo.oOo.%
    ...
    Nd.Z = [zeros(1, nBM), ...
                zeros(1,nCC), 0, ...
                    0, z_tilt, 0.5*z_tilt, z_tilt, z_tilt, z_tilt, z_tilt, z_tilt, z_tilt, z_tilt, z_tilt]; % Tilting of OHC and DC process in longitudinal direction
    ...
    for ii = 1:size(Nd.X,2)
        ...
        elseif ii == nBM + nCC + 9
            Nd.name(:,ii) = {'E4'};
        elseif ii == nBM + nCC + 10           
            Nd.name(:,ii) = {'E5'};
        elseif ii == nBM + nCC + 11           
            Nd.name(:,ii) = {'E0'};
        elseif ii == nBM + nCC + 12           
            Nd.name(:,ii) = {'PP'};
        else
            Nd.name(:,ii) = {'XX'};
        end        
    ...
    for iz=1:size(Nd.X,1)
        for ix=1:size(Nd.X,2)
            if ix<nBM && abs(p.A1(iz,1)-Nd.X(iz,ix))<1e-3
                Nd.name(iz,ix) = {'AP'};
            elseif ix<nBM && abs(p.A2(iz,1)-Nd.X(iz,ix))<1e-3
                Nd.name(iz,ix) = {'AM'};
            end
        end
    end
