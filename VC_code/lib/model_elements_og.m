function El = model_elements_og(Nd, MP)
...
% #
    ...
    for iz = 1:nz,  % section number along the longitudinal directon (z-dir)
        ...
        for jj = 1:length(nd1),
            if mean(Nd.X([nd1(jj), nd2(jj)])) < qMP.width_BM(iz)*(3/10),    % [YJL, 2013.11.01]
        ...
        c1 = (c1-1)*nz + iz; c2 = (c2-1)*nz + iz; dd = (dd-1)*nz + iz;
        cc = (cc-1)*nz + iz; bb = (bb-1)*nz + iz; 
        ff = (ff-1)*nz + iz;
        ...
        a1 = (kAP-1)*nz + iz; % root of OPC
        a2 = (kAM-1)*nz + iz; % root of DC
        
        if rem(abs(Nd.Z(iz,1)),10)<0.1
            % % Pillar Cells
            if ~isempty(c1),
                El.Nd1 = [El.Nd1; a0; a1; c1;  c2];
                El.Nd2 = [El.Nd2; c1; c2; cc;  cc];
                El.name = [El.name; {'IPC'}; {'OPC'}; {'IPC'}; {'OPC'};];
            else
                El.Nd1 = [El.Nd1; a0; a1];
                El.Nd2 = [El.Nd2; cc; cc];
            ...
            if MP.tilt_OHC>=0 && nz-iz >= MP.tilt_OHC,
                El.Nd1 = [El.Nd1; a2; dd; dd];
                El.Nd2 = [El.Nd2; dd; bbp; bb];
                El.name = [El.name; {'DCb'}; {'DCp'};{'OHC'}];
        ...
        El.name = [El.name; {'TMx2'}; {'TMx2'}; {'TMx2'}; {'TMx2'}; {'TMx1'};]; % 1/4 attachement zoen, 1/4 body zone
% %         El.name = [El.name; {'TMx2'}; {'TMx2'}; {'TMx2'}; {'TMx1'}; {'TMx1'};]; % 1/2 attachement zone, 1/2 body zone
