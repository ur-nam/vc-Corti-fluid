function [Nd, MP] = model_nodes_og(MP)
...
% %     p.A2 = [1/2*qMP.width_BM, zeros(nz,1)];  % root of DC
    ...
    for iz=1:nz,
        ...
        bb = R1*([qMP.width_BM(iz)/10;0]) + p.CC(iz,:)';   % root of OHC bundle or apex of OHC
    ...
    for iz=1:nz,
        ...
        R1 = [cos(q1), -sin(q1); sin(q1), cos(q1)];
    %     q1 = 10*pi/180;     R1 = [cos(q1), -sin(q1); sin(q1), cos(q1)];
...
% % 
% %     p.E2 = 13/16*p.E1 + 3/16*p.E6;
% %     p.E3 = 10/16*p.E1 + 6/16*p.E6;
% %     p.E4 = 7/16*p.E1 + 9/16*p.E6;
% %     p.E5 = 4/16*p.E1 + 12/16*p.E6;
% %     
...
function     Nd = assign_nodal_coordinates(p, MP)
    ...
    Nd.X = zeros(nz,nBM+12);
    Nd.Y = zeros(nz,nBM+12);
    Nd.Z = zeros(nz,nBM+12);
    ...
    for iz=1:nz,    
        ...
        Nd.X(iz,:) = [nodes_arcuate, nodes_IPC_DC_roots, nodes_pectinate,...
                    0.5*(p.A0(iz,1)+p.CC(iz,1)),0.5*(p.A1(iz,1)+p.CC(iz,1)), p.DD(iz,1), ...
                        p.CC(iz,1), p.BB(iz,1), p.FF(iz,1), p.E1(iz,1), p.E2(iz,1), p.E3(iz,1), p.E4(iz,1), p.E5(iz,1), p.E6(iz,1)];
        Nd.Y(iz,:) = [zeros(1,nBM),...
                    0.5*(p.A0(iz,2)+p.CC(iz,2)),0.5*(p.A1(iz,2)+p.CC(iz,2)), p.DD(iz,2), ...
                        p.CC(iz,2), p.BB(iz,2), p.FF(iz,2), p.E1(iz,2), p.E2(iz,2), p.E3(iz,2), p.E4(iz,2), p.E5(iz,2), p.E6(iz,2)];
    end
...
% %     %.oOo.oOo.oOo.oOo.oOo.oOo.oOo.oOo.oOo.oOo.oOo.oOo.%
    ...
    Nd.Z = [zeros(1, nBM), ...
                0, 0, 0, ...
                    0,  z_tilt, z_tilt, z_tilt, z_tilt, z_tilt, z_tilt, z_tilt, z_tilt]; % Tilting of OHC and DC process in longitudinal direction
                
    zz = (0:MP.dZ:MP.length_BM)' - round(0.5*MP.length_BM/10)*10;    
